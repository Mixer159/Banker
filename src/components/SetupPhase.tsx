  ...
  return (
    ...
    >
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <AnimatePresence mode="popLayout">
            {clients.map((client) => (
              <ClientCardSetup
                ...
                need={client.need}
                onChangeMax={(val) =>
                  onUpdateClient(client.id, val, client.allocated)
                }
                onChangeAllocated={(val) =>
                  onUpdateClient(client.id, client.max, val)
                }
          ...
          {clients.length < 4 && (
            <motion.button
              ...
              className="flex min-h-[200px] flex-col items-center justify-center gap-3 rounded-xl border-2 border-dashed border-amber-300 bg-amber-50/50 text-amber-600 transition-colors hover:border-amber-400 hover:bg-amber-50 hover:text-amber-700"
      ...
      >
        <Button
          onClick={onStart}
          className="h-12 gap-2 rounded-xl bg-amber-600 px-8 text-base font-semibold text-white shadow-lg transition-all hover:bg-amber-700 hover:shadow-xl"
        >
